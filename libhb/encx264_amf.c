...
int  encx264_amfInit( hb_work_object_t *, hb_job_t * );
int  encx264_amfWork( hb_work_object_t *, hb_buffer_t **, hb_buffer_t ** );
void encx264_amfClose( hb_work_object_t * );

hb_work_object_t hb_encx264_amf =
{
    WORK_ENCX264_AMF,
    "H.264/AVC encoder (AMF)",
    encx264_amfInit,
    encx264_amfWork,
    encx264_amfClose
};
...
static x264_amf_api_t x264_amf_apis[HB_X264_API_COUNT];

const char *libx264_amf_10bit_names[] = {
    "libx26410b", "libx264_main10", NULL
};

const char *libx264_amf_8bit_names[] = {
...
void hb_x264_amf_global_init()
{
#if X264_BUILD < 153
    x264_amf_apis[0].bit_depth                 = x264_bit_depth;
#else
    x264_amf_apis[0].bit_depth                 = X264_BIT_DEPTH;
#endif
    x264_amf_apis[0].param_default             = x264_param_default;
    x264_amf_apis[0].param_default_preset      = x264_param_default_preset;
    x264_amf_apis[0].param_apply_profile       = x264_param_apply_profile;
    x264_amf_apis[0].param_apply_fastfirstpass = x264_param_apply_fastfirstpass;
    x264_amf_apis[0].param_parse               = x264_param_parse;
    x264_amf_apis[0].encoder_open              = x264_encoder_open;
    x264_amf_apis[0].encoder_headers           = x264_encoder_headers;
    x264_amf_apis[0].encoder_encode            = x264_encoder_encode;
    x264_amf_apis[0].encoder_delayed_frames    = x264_encoder_delayed_frames;
    x264_amf_apis[0].encoder_close             = x264_encoder_close;
    x264_amf_apis[0].picture_init              = x264_picture_init;

    if (x264_amf_apis[0].bit_depth == 0)
    {
        // libx264 supports 8 and 10 bit
        x264_amf_apis[0].bit_depth                 = 8;
        x264_amf_apis[1].bit_depth                 = 10;
        x264_amf_apis[1].param_default             = x264_param_default;
        x264_amf_apis[1].param_default_preset      = x264_param_default_preset;
        x264_amf_apis[1].param_apply_profile       = x264_param_apply_profile;
        x264_amf_apis[1].param_apply_fastfirstpass = x264_param_apply_fastfirstpass;
        x264_amf_apis[1].param_parse               = x264_param_parse;
        x264_amf_apis[1].encoder_open              = x264_encoder_open;
        x264_amf_apis[1].encoder_headers           = x264_encoder_headers;
        x264_amf_apis[1].encoder_encode            = x264_encoder_encode;
        x264_amf_apis[1].encoder_delayed_frames    = x264_encoder_delayed_frames;
        x264_amf_apis[1].encoder_close             = x264_encoder_close;
        x264_amf_apis[1].picture_init              = x264_picture_init;
        return;
    }

    // Invalidate other apis
    x264_amf_apis[1].bit_depth = -1;

    // Attempt to dlopen a library for handling the bit-depth that we do
    // not already have.
    void *h;
    if (x264_amf_apis[0].bit_depth == 8)
    {
        h = x264_lib_open(libx264_amf_10bit_names);
#if defined(SYS_LINUX)
        if (h == NULL)
        {
    ...
    {
        h = x264_lib_open(libx264_amf_8bit_names);
    }
    if (h == NULL)
    {
        return;
...
#endif
    x264_amf_apis[1].param_default        = hb_dlsym(h, "x264_param_default");
#if X264_BUILD >= 153
    if (x264_amf_apis[1].param_default != NULL)
    {
        x264_param_t defaults;
        x264_amf_apis[1].param_default(&defaults);
        dll_bitdepth = defaults.i_bitdepth;
    }
#endif
    x264_amf_apis[1].param_default_preset = hb_dlsym(h, "x264_param_default_preset");
    x264_amf_apis[1].param_apply_profile  = hb_dlsym(h, "x264_param_apply_profile");
    x264_amf_apis[1].param_apply_fastfirstpass =
                                hb_dlsym(h, "x264_param_apply_fastfirstpass");
    x264_amf_apis[1].param_parse          = hb_dlsym(h, "x264_param_parse");
    ...
    for (ii = 140; ii < 200; ii++)
    {
        ...
        x264_amf_apis[1].encoder_open = hb_dlsym(h, name);
        free(name);
        if (x264_amf_apis[1].encoder_open != NULL)
        {
            break;
        }
    }
    x264_amf_apis[1].encoder_headers      = hb_dlsym(h, "x264_encoder_headers");
    x264_amf_apis[1].encoder_encode       = hb_dlsym(h, "x264_encoder_encode");
    x264_amf_apis[1].encoder_delayed_frames =
                                hb_dlsym(h, "x264_encoder_delayed_frames");
    x264_amf_apis[1].encoder_close        = hb_dlsym(h, "x264_encoder_close");
    x264_amf_apis[1].picture_init         = hb_dlsym(h, "x264_picture_init");

    if (dll_bitdepth > 0 && dll_bitdepth != x264_amf_apis[0].bit_depth &&
        x264_amf_apis[1].param_default             != NULL &&
        x264_amf_apis[1].param_default_preset      != NULL &&
        x264_amf_apis[1].param_apply_profile       != NULL &&
        x264_amf_apis[1].param_apply_fastfirstpass != NULL &&
        x264_amf_apis[1].param_parse               != NULL &&
        x264_amf_apis[1].encoder_open              != NULL &&
        x264_amf_apis[1].encoder_headers           != NULL &&
        x264_amf_apis[1].encoder_encode            != NULL &&
        x264_amf_apis[1].encoder_delayed_frames    != NULL &&
        x264_amf_apis[1].encoder_close             != NULL &&
        x264_amf_apis[1].picture_init              != NULL)
    {
        x264_amf_apis[1].bit_depth = dll_bitdepth;
    }
}

const x264_amf_api_t *hb_x264_amf_api_get(int bit_depth)
{
    int ii;

    for (ii = 0; ii < HB_X264_API_COUNT; ii++)
    {
        if (-1        != x264_amf_apis[ii].bit_depth &&
            bit_depth == x264_amf_apis[ii].bit_depth)
        {
            return &x264_amf_apis[ii];
        }
    }
    return NULL;
}
 ...
 */
static int apply_h264_level(const x264_amf_api_t *api, x264_param_t *param,
 ...
 */
static int apply_h264_profile(const x264_amf_api_t *api, x264_param_t *param,
                              const char *h264_profile, int verbose);

/***********************************************************************
 * hb_work_encx264_amf_init
 ***********************************************************************
 *
 **********************************************************************/
int encx264_amfInit( hb_work_object_t * w, hb_job_t * job )
{
    ...
    if (pv->api == NULL)
    {
        hb_error("encx264: hb_x264_amf_api_get failed, bit-depth %d", bit_depth);
        return 1;
    }
    ...
    char *x264_opts_unparsed = hb_x264_amf_param_unparse(pv->api->bit_depth,
                                                     job->encoder_preset,
                                                     job->encoder_tune,
                                                     job->encoder_options,
                                                     job->encoder_profile,
...
void encx264_amfClose( hb_work_object_t * w )
{
...
int encx264_amfWork( hb_work_object_t * w, hb_buffer_t ** buf_in,
                  hb_buffer_t ** buf_out )
{
    hb_work_private_t *pv = w->private_data;
...
int apply_h264_level(const x264_amf_api_t *api, x264_param_t *param,
                             ...
                             int width, int height)
{
     ...
     */
    api = hb_x264_amf_api_get(bit_depth);
...
{
    int i;
    for (i = 0; hb_x264_amf_encopt_synonyms[i][0] != NULL; i++)
        if (!strcmp(name, hb_x264_amf_encopt_synonyms[i][1]))
            return hb_x264_amf_encopt_synonyms[i][0];
    return name;
}
